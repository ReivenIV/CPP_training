...
ClapTrap::ClapTrap(std::string name) : _name(name), _hit_points(10), _energy_points(10), _damage_points(0) {
	std::cout << " constructor named: " << name << std::endl;
}
...
void ClapTrap::takeDamage(unsigned int amount) {
	if (_hit_points < amount) {
		std::cout << "⛑️   " << _name << " is out of combat" << std::endl;
		_hit_points = 0;
		return ;
	}
	std::cout << "⛑️   " << _name << " take damage causing " << amount << " hit points lost !" << std::endl;
	_hit_points = _hit_points - amount;	
}

void ClapTrap::beRepaired(unsigned int amount) {
	if (_energy_points <= 0) {
		std::cout << "⛑️   " << _name << " has no more energy left need recharge ASAP" << std::endl;
		return ;
	}
	std::cout << "⛑️   " << _name << " get repaired  causing " << amount << " energie points gained !" << std::endl;
	_hit_points = _hit_points + amount;
	_energy_points--;
}
